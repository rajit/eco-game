...
public class Pathfinder2D : MonoBehaviour 
{
    ...
    private void Create2DMap ()
	{
		...
		// A* Pathfinding tutorial on youtube
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				int ID = (y * width) + x; //ID we give to our Node!
				Vector3 worldPoint = worldBottomLeft + Vector3.right * (x * Tilesize + tileRadius) + Vector3.up * (y * Tilesize + tileRadius);
				bool walkable = !(Physics.CheckSphere(worldPoint, .5f, unwalkableMask.value));
				newMap [x, y] = new Node (x, y, worldPoint.y, ID, worldPoint.x, 0, walkable);
			}
		}
